...
with col1:
    st.markdown("**Сервер**")
    st.write(
        f"CPU: {chosen.cores_label}

"
        f"RAM: {chosen.ram_gb} ГБ

"
        f"ОС: {OS_NAME}

"
        f"Подсистема ОС: {OS_STORAGE_STR}"
    )

with col2:
    st.markdown("**Хранилище видеоархива**")
    st.write(
        f"Требуемый объём на 1 камеру: {ARCHIVE_TB_PER_CAMERA:.1f} ТБ

"
        f"Камер: {cams}

"
        f"Итого требуемо: {archive_effective_tb:.2f} ТБ"
    )

with col3:
    st.markdown("**Дисковый массив (под архив)**")
    st.write(
        f"Диск: {disk_tb:.0f} ТБ

"
        f"Базовых дисков (data+parity): {plan['base_disks']}

"
        f"Hot‑spare (для >16 дисков, 1 на 18): {plan['spares']}
...
"
        f"Полезная ёмкость: {plan['usable_tb']:.2f} ТБ

"
        f"Требуемая ёмкость: {plan['required_tb']:.2f} ТБ

"
        f"Суммарная RAW-ёмкость: {plan['raw_tb']:.2f} ТБ"
    )

st.divider()

st.caption(
    "Правила подбора CPU/RAM, подсистемы ОС и массива под архив жёстко соответствуют упрощённому ТЗ из чата.
"
